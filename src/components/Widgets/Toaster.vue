...
const props = defineProps({
  ...
  type: {
    type: String,
    default: 'error',
    validator: (val) => ['error', 'warning', 'info'].includes(val)
  },
  canClose: {
    type: Boolean,
    default: true
...
.absolute.top-0.left-0.right-0(
  ...
  .m-auto.flex.max-w-min.relative.my-5.items-center.p-4.text-gray-500.bg-white.rounded-lg.shadow
    .inline-flex.flex-shrink-0.justify-center.items-center.w-8.h-8.rounded-lg(
      :class="{ 'text-red-500 bg-red-100': type === 'error', 'text-orange-500 bg-orange-100': type === 'warning', 'text-gray-500 bg-gray-100': type === 'info' }"
    )
      svg(
        aria-hidden="true"
